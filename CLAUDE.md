...
- `SpanWallpaper` (`Sources/SpanWallpaper/`) -- the app, split into 10 files: `main.swift` (entry point), `AppDelegate.swift`, `ScreenLayout.swift`, `ImagePipeline.swift`, `WallpaperSetter.swift`, `Rotation.swift`, `PreferencesUI.swift`, `StatusBarController.swift`, `AboutPanel.swift`, `BuildInfo.swift` (generated), `Utilities.swift`.
...
- Debug log: `~/Library/Application Support/SpanWallpaper/debug.log` (opt-in via Debug section in test builds; timestamped transition log)
...
- **BuildInfo**: `BuildInfo.swift` is generated by `setup.sh` and `build-pkg.sh` before compiling — injects version (from `VERSION` file) and build date. The checked-in copy has placeholder values ("dev"/"unknown") so bare `swift build` still works.
- **DEBUG_UI compile flag**: test builds (`./build-pkg.sh`, `./setup.sh`) pass `-Xswiftc -DDEBUG_UI`; signed release builds (`./build-pkg.sh --sign`) do not. The collapsible Debug section in preferences is wrapped in `#if DEBUG_UI`.
- **Preinstall script**: `pkg-scripts/preinstall` gracefully quits a running SpanWallpaper before the installer replaces the binary — osascript quit with 5s timeout, then SIGKILL fallback.
