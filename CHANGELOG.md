...
### Features
- **About window**: version and build date shown via "About SpanWallpaper" in the menu bar menu
- **Debug section** (test builds only): collapsible section in preferences with enable checkbox and "View Log" button that opens the debug log in the default text editor
- **Preinstall hook**: pkg installer gracefully quits the running app before replacing it
...
### Under the hood
- `BuildInfo.swift` generated by build scripts with version and date at compile time
- `DEBUG_UI` compile flag: test/dev builds get `-DDEBUG_UI`, signed release builds do not
- `pkg-scripts/preinstall` sends osascript quit, waits 5s, then force kills if needed

## 2.3.1 — 2026-05-19

Bug fixes for retire and pkg installer (same fixes as 2.4.1, released before About/Debug features).
